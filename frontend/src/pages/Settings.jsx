...
export default function Settings() {
    ...
    const [schoolData, setSchoolData] = useState({ name: "", short_name: "", email: "", phone: "", address: "", school_type: "Combined" });
    ...
    async function loadData() {
        ...
        try {
            if (isPlatformAdmin) {
                ...
                const sInfo = res.data?.data || res.data || school;
                setSchoolData({ name: sInfo.name || "", short_name: sInfo.short_name || "", email: sInfo.email || "", phone: sInfo.phone || "", address: sInfo.address || "", school_type: sInfo.school_type || "Combined" });
            }
    ...
    async function handleSave() {
        try {
            ...
            else if (school) await api.put(`/schools/${school.id}`, schoolData);
    ...
    if (!isPlatformAdmin) {
        return (
            <PageContainer>
                <PageHeader title="School Settings" subtitle="Manage your school profile" action={<button onClick={handleSave} disabled={saving} className="bg-amber-600 text-white px-6 py-2.5 rounded-xl font-bold shadow-lg hover:bg-amber-700 disabled:opacity-50">{saving ? "Saving..." : "Save School Settings"}</button>} />
                ...
                <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm space-y-6">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div className="space-y-2"><label className="text-sm font-semibold text-slate-600">School Email</label><input type="email" className="w-full p-3 rounded-xl border border-slate-200 focus:ring-2 focus:ring-amber-500 outline-none" value={schoolData.email} onChange={e => setSchoolData({...schoolData, email: e.target.value})} /></div>
                        <div className="space-y-2"><label className="text-sm font-semibold text-slate-600">School Phone</label><input className="w-full p-3 rounded-xl border border-slate-200 focus:ring-2 focus:ring-amber-500 outline-none" value={schoolData.phone} onChange={e => setSchoolData({...schoolData, phone: e.target.value})} /></div>
                        <div className="space-y-2 md:col-span-2"><label className="text-sm font-semibold text-slate-600">School Address</label><textarea className="w-full p-3 rounded-xl border border-slate-200 focus:ring-2 focus:ring-amber-500 outline-none" rows={3} value={schoolData.address} onChange={e => setSchoolData({...schoolData, address: e.target.value})} /></div>
