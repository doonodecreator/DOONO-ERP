...
export default function Settings() {
    ...
    async function loadData() {
        ...
        try {
            if (isPlatformAdmin) {
                ...
                setSchoolData({
                    name: sInfo.name || "",
                    short_name: sInfo.short_name || "",
                    email: sInfo.email || "",
                    phone: sInfo.phone || "",
                    address: sInfo.address || "",
                    school_type: sInfo.school_type || "Combined",
                    motto: opsData.motto || "",
                    bank_name: opsData.bank_name || "",
                    account_number: opsData.account_number || "",
                    account_name: opsData.account_name || "",
                    paystack_public_key: opsData.paystack_public_key || "",
                    paystack_secret_key: opsData.paystack_secret_key || "",
                    paystack_subaccount_code: opsData.paystack_subaccount_code || ""
                });
            }
        } catch (err) {
    ...
    async function handleSave() {
        try {
            ...
            if (isPlatformAdmin) {
                // Ensure we send IDs, not objects
                const payload = {
                    ...settings,
                    default_subscription_plan_id: settings.default_subscription_plan_id,
                ...
                await Promise.all([
                    ...
                    api.put("/school-settings", {
                        motto: schoolData.motto,
                        bank_name: schoolData.bank_name,
                        account_number: schoolData.account_number,
                        account_name: schoolData.account_name,
                        paystack_public_key: schoolData.paystack_public_key,
                        paystack_secret_key: schoolData.paystack_secret_key,
                        paystack_subaccount_code: schoolData.paystack_subaccount_code
                    })
                ]);
            }
                    ...
                    <section className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div className="space-y-2">
                                ...
                                <input className="w-full p-3 rounded-xl border border-slate-200 focus:ring-2 focus:ring-amber-500 outline-none" value={schoolData.name} onChange={e => setSchoolData({...schoolData, name: e.target.value})} />
                            </div>
                            <div className="space-y-2">
                                <label className="text-sm font-semibold text-slate-600">Short Name / Acronym</label>
                                <input className="w-full p-3 rounded-xl border border-slate-200 focus:ring-2 focus:ring-amber-500 outline-none" value={schoolData.short_name} onChange={e => setSchoolData({...schoolData, short_name: e.target.value})} />
                            </div>
                            <div className="space-y-2">
                                <label className="text-sm font-semibold text-slate-600">School Motto</label>
                                <input className="w-full p-3 rounded-xl border border-slate-200 focus:ring-2 focus:ring-amber-500 outline-none" value={schoolData.motto} onChange={e => setSchoolData({...schoolData, motto: e.target.value})} placeholder="e.g. Excellence in Learning" />
                            </div>
                            <div className="space-y-2">
                                <label className="text-sm font-semibold text-slate-600">School Type</label>
                                <select className="w-full p-3 rounded-xl border border-slate-200 focus:ring-2 focus:ring-amber-500 outline-none" value={schoolData.school_type} onChange={e => setSchoolData({...schoolData, school_type: e.target.value})}>
                            ...
                            <div className="space-y-2">
                                <label className="text-sm font-semibold text-slate-600">School Email</label>
                                <input type="email" className="w-full p-3 rounded-xl border border-slate-200 focus:ring-2 focus:ring-amber-500 outline-none" value={schoolData.email} onChange={e => setSchoolData({...schoolData, email: e.target.value})} />
                            </div>
                            <div className="space-y-2">
                                <label className="text-sm font-semibold text-slate-600">School Phone</label>
                                <input className="w-full p-3 rounded-xl border border-slate-200 focus:ring-2 focus:ring-amber-500 outline-none" value={schoolData.phone} onChange={e => setSchoolData({...schoolData, phone: e.target.value})} />
                            </div>
                            <div className="space-y-2 md:col-span-2">
                                <label className="text-sm font-semibold text-slate-600">School Address</label>
                                <textarea className="w-full p-3 rounded-xl border border-slate-200 focus:ring-2 focus:ring-amber-500 outline-none" rows={2} value={schoolData.address} onChange={e => setSchoolData({...schoolData, address: e.target.value})} />
                            </div>
                        </div>
                    </section>
                    ...
                    <section className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div className="space-y-2">
                                ...
                                <input className="w-full p-3 rounded-xl border border-slate-200 focus:ring-2 focus:ring-amber-500 outline-none" value={schoolData.bank_name} onChange={e => setSchoolData({...schoolData, bank_name: e.target.value})} />
                            </div>
                            <div className="space-y-2">
                                <label className="text-sm font-semibold text-slate-600">Account Number</label>
                                <input className="w-full p-3 rounded-xl border border-slate-200 focus:ring-2 focus:ring-amber-500 outline-none" value={schoolData.account_number} onChange={e => setSchoolData({...schoolData, account_number: e.target.value})} />
                            </div>
                            <div className="space-y-2 md:col-span-2">
                                <label className="text-sm font-semibold text-slate-600">Account Name</label>
                                <input className="w-full p-3 rounded-xl border border-slate-200 focus:ring-2 focus:ring-amber-500 outline-none" value={schoolData.account_name} onChange={e => setSchoolData({...schoolData, account_name: e.target.value})} />
                            </div>
                            <div className="space-y-2">
                                <label className="text-sm font-semibold text-slate-600">Paystack Public Key</label>
                                <input className="w-full p-3 rounded-xl border border-slate-200 focus:ring-2 focus:ring-amber-500 outline-none" value={schoolData.paystack_public_key} onChange={e => setSchoolData({...schoolData, paystack_public_key: e.target.value})} />
                            </div>
                            <div className="space-y-2">
                                <label className="text-sm font-semibold text-slate-600">Paystack Subaccount Code</label>
                                <input className="w-full p-3 rounded-xl border border-slate-200 focus:ring-2 focus:ring-amber-500 outline-none" value={schoolData.paystack_subaccount_code} onChange={e => setSchoolData({...schoolData, paystack_subaccount_code: e.target.value})} placeholder="ACCT_xxxxxxxxx" />
                            </div>
                        </div>
                    </section>
                </div>
                    ...
                    <div className="space-y-4">
                        <div className="flex items-center justify-between p-4 bg-slate-50 rounded-xl">
                            <div>
                                ...
                                <p className="text-xs text-slate-500">Allow schools to register themselves</p>
                            </div>
